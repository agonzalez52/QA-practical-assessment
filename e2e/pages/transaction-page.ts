...
export class TransactionPage {
  ...
  constructor(page: Page) {
    this.page = page;
    this.contactItem = (contactId: string) =>
      page.locator(`[data-test="user-list-item-${contactId}"]`);
    this.transactionAmountField = page.locator(
      '[data-test="transaction-create-amount-input"] input'
    );
    this.transactionNoteField = page.locator(
  ...
  async clickPayButtonAndGetTransactionId(): Promise<string> {
    const [response] = await Promise.all([
      this.page.waitForResponse(
        (resp) => resp.url().includes("/transactions") && resp.request().method() === "POST"
      ),
